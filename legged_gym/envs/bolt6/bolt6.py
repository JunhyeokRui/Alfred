...
class Bolt6(LeggedRobot):
    ...
    def _compute_torques(self, actions):
        ...
        if control_type=="P":
            ...
            k36= -1.5752
            q36= -0.0254
            k_joint= torch.tensor([k14, k25 , k36 , k14 , k25, k36], device=self.device)
            q_init = torch.tensor([q14, q25 , q36 ,-q14 , q25, q36], device=self.device)
            spring_torque = k_joint*(actions - q_init)
            torques = actions_scaled + spring_torque ####### HERE IS PLUS
            # print(control_type)
            # print(spring_torque)
